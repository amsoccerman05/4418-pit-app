...
for(const width of [390,1440])test(`signed-out gateway has no protected-content flash ${width}`,async({page})=>{
 ...
 await page.route('**/src/client.ts',r=>r.fulfill({contentType:'application/javascript',body: `import {createSuiteClient} from '/src/suite-auth.ts'; export const configured=true; export const configError=''; export const supabase=createSuiteClient('https://pit-test.supabase.invalid','test-public-key');`}));
